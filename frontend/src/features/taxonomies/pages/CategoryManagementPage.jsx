...
export default function CategoryManagementPage() {
  ...
  const {
    ...
    handleToggleActive,
    retry,
  } = useTaxonomyManagement('category')
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">Create, edit, activate, or deactivate blog post categories.</p>
        </div>
      ...
      {error ? (
        <div className="rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
          <p className="font-semibold">{error.message || 'Failed to load categories.'}</p>
          <button
      ...
      {isLoading ? (
        <div aria-label="Loading categories" className="p-12 text-center text-slate-500" role="status">
        ...
        <>
          ...
          {data && data.count > 0 ? (
            <div className="flex items-center justify-between border-t border-slate-200 pt-4 text-sm">
              <span className="text-slate-500">Total: {data.count} categories</span>
